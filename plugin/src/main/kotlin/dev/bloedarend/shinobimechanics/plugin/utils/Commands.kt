...
import dev.bloedarend.shinobimechanics.plugin.commands.Default
import dev.bloedarend.shinobimechanics.plugin.commands.Help
import dev.bloedarend.shinobimechanics.plugin.commands.Reload
...
class Commands(private val configs: Configs, private val events: Events, private val messages: Messages, private val plugin: Plugin) {
    ...
    private fun registerExceptions() {
        bukkitCommandHandler.registerExceptionHandler(CommandInvocationException::class.java) { actor, _ ->
            messages.sendMessage("exceptions.error", (actor as BukkitCommandActor).sender as Player)
        }

        bukkitCommandHandler.registerExceptionHandler(InvalidBooleanException::class.java) { actor, exception ->
            messages.sendMessage("errors.invalid-boolean", (actor as BukkitCommandActor).sender as Player,
        ...
        bukkitCommandHandler.registerExceptionHandler(InvalidCommandException::class.java) { actor, exception ->
            messages.sendMessage("errors.invalid-command", (actor as BukkitCommandActor).sender as Player,
                Pair("%input%", exception.input)
            )
        }

        bukkitCommandHandler.registerExceptionHandler(InvalidNumberException::class.java) { actor, exception ->
            messages.sendMessage("errors.invalid-number", (actor as BukkitCommandActor).sender as Player,
                Pair("%input%", exception.input)
            )
        }

        bukkitCommandHandler.registerExceptionHandler(InvalidPlayerException::class.java) { actor, exception ->
            messages.sendMessage("errors.invalid-player", (actor as BukkitCommandActor).sender as Player,
                Pair("%input%", exception.input)
            )
        }
        ...
        bukkitCommandHandler.registerExceptionHandler(NoPermissionException::class.java) { actor, _ ->
            messages.sendMessage("exceptions.no-permission", (actor as BukkitCommandActor).sender as Player)
        }

        bukkitCommandHandler.registerExceptionHandler(NoSubcommandSpecifiedException::class.java) { actor, _ ->
            messages.sendMessage("exceptions.no-subcommand", (actor as BukkitCommandActor).sender as Player)
        }

        bukkitCommandHandler.registerExceptionHandler(NumberNotInRangeException::class.java) { actor, exception ->
            messages.sendMessage("errors.number-not-in-range", (actor as BukkitCommandActor).sender as Player,
        ...
        bukkitCommandHandler.registerExceptionHandler(MissingArgumentException::class.java) { actor, exception ->
            messages.sendMessage("errors.missing-argument", (actor as BukkitCommandActor).sender as Player,
        ...
        bukkitCommandHandler.registerExceptionHandler(SenderNotConsoleException::class.java) { actor, exception ->
            messages.sendMessage("exceptions.sender-not-console", (actor as BukkitCommandActor).sender as Player)
        }

        bukkitCommandHandler.registerExceptionHandler(SenderNotPlayerException::class.java) { actor, exception ->
            messages.sendMessage("exceptions.sender-not-player", (actor as BukkitCommandActor).sender as Player)
        }

        bukkitCommandHandler.registerExceptionHandler(TooManyArgumentsException::class.java) { actor, exception ->
            messages.sendMessage("errors.incorrect-usage", (actor as BukkitCommandActor).sender as Player,
